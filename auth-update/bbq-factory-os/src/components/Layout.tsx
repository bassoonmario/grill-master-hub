    ...
    <div className="min-h-screen bg-bg flex flex-col">
      ...
      <header className="sticky top-0 z-40 bg-surface border-b border-border flex items-center justify-between px-5 py-3">
        ...
        <div className="flex items-center gap-2.5">
          ...
          <button
            onClick={logout}
            className="text-[var(--text-dim)] hover:text-[var(--red)] transition-colors bg-transparent border-none cursor-pointer text-lg p-1"
            title="Вийти"
          >
            ⏻
          </button>
      ...
      {/* Page content */}
      <main className="flex-1 overflow-y-auto pb-24 px-5 pt-5 animate-fade-in">
        {children}
      </main>
